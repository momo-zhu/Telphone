...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public PullToRefreshAdapterViewBase(Context context, PtrMode ptrMode) {
		super(context, ptrMode);
		mRefreshableView.setOnScrollListener(this);
	}

	public PullToRefreshAdapterViewBase(Context context, PtrMode ptrMode, AnimationStyle animStyle) {
		super(context, ptrMode, animStyle);
		mRefreshableView.setOnScrollListener(this);
	}
	...
	protected void updateUIForMode() {
		...
		// Check Indicator Views consistent with new PtrMode
		if (getShowIndicatorInternal()) {
			addIndicatorViews();
		} else {
			removeIndicatorViews();
	...
	private void addIndicatorViews() {
		PtrMode ptrMode = getMode();
		FrameLayout refreshableViewWrapper = getRefreshableViewWrapper();

		if (ptrMode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			// If the ptrMode can pull down, and we don't have one set already
		...
		} else if (!ptrMode.showHeaderLoadingLayout() && null != mIndicatorIvTop) {
			// If we can't pull down, but have a View then remove it
			refreshableViewWrapper.removeView(mIndicatorIvTop);
			mIndicatorIvTop = null;
		}

		if (ptrMode.showFooterLoadingLayout() && null == mIndicatorIvBottom) {
			// If the ptrMode can pull down, and we don't have one set already
		...
		} else if (!ptrMode.showFooterLoadingLayout() && null != mIndicatorIvBottom) {
			// If we can't pull down, but have a View then remove it
