...
import com.handmark.pulltorefresh.library.PullToRefreshBase.PtrMode;
import com.handmark.pulltorefresh.library.PullToRefreshBase.PtrState;
...
public final class OverscrollHelper {
	...
	public static void overScrollBy(final PullToRefreshBase<?> view, final int deltaX, final int scrollX,
		...
		if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
			final PtrMode ptrMode = view.getMode();

			// Check that Pull-to-Refresh is enabled, and the event isn't from
			// touch
			if (ptrMode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					// Check the ptrMode supports the overscroll direction, and
					// then move scroll
					if (ptrMode.showHeaderLoadingLayout()) {
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
							view.setState(PullToRefreshBase.PtrState.OVERSCROLLING);
						}
					...
					// Check the ptrMode supports the overscroll direction, and
					// then move scroll
					if (ptrMode.showFooterLoadingLayout()) {
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
							view.setState(PtrState.OVERSCROLLING);
						}
					...
					view.setState(PtrState.RESET);
				}
			} else if (isTouchEvent && PtrState.OVERSCROLLING == view.getState()) {
